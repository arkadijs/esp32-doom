...
static int StreamGetc(midi_stream_t *stream)
{
    if (stream->position >= stream->length) return EOF;
    return stream->data[stream->position++];
}

static size_t StreamRead(void *ptr, size_t size, size_t count,
                         midi_stream_t *stream)
{
    ...
    items = count;
    if (items > available / size) items = available / size;
    memcpy(ptr, stream->data + stream->position, items * size);
    stream->position += items * size;
    return items;
}

static int StreamSeek(midi_stream_t *stream, long offset, int whence)
{
    long position;
    if (whence == SEEK_SET) position = offset;
    else if (whence == SEEK_CUR) position = (long) stream->position + offset;
    else if (whence == SEEK_END) position = (long) stream->length + offset;
    else return -1;
...
static boolean ReadByte(byte *result, midi_stream_t *stream)
{
    int c;
...
static boolean ReadVariableLength(unsigned int *result, midi_stream_t *stream)
{
    int i;
    byte b = 0;
...
static void *ReadByteSequence(unsigned int num_bytes, midi_stream_t *stream)
{
    unsigned int i;
    byte *result;
...
static boolean ReadMetaEvent(midi_event_t *event, midi_stream_t *stream)
{
    byte b = 0;
                         ...
                         midi_stream_t *stream)
{
    ...
    if ((event_type & 0x80) == 0)
    {
        ...
        if (StreamSeek(stream, -1, SEEK_CUR) < 0)
        {
...
static boolean ReadTrackHeader(midi_track_t *track, midi_stream_t *stream)
{
    size_t records_read;
    chunk_header_t chunk_header;

    records_read = StreamRead(&chunk_header, sizeof(chunk_header_t), 1, stream);
    ...
    track->data_len = __builtin_bswap32(chunk_header.chunk_size);

    return true;
}

static boolean ReadTrack(midi_track_t *track, midi_stream_t *stream)
{
...
static boolean ReadAllTracks(midi_file_t *file, midi_stream_t *stream)
{
    unsigned int i;
...
static boolean ReadFileHeader(midi_file_t *file, midi_stream_t *stream)
{
    size_t records_read;
    unsigned int format_type;

    records_read = StreamRead(&file->header, sizeof(midi_header_t), 1, stream);
    ...
    if (!CheckChunkHeader(&file->header.chunk_header, HEADER_CHUNK_ID)
     || __builtin_bswap32(file->header.chunk_header.chunk_size) != 6)
    {
        fprintf(stderr, "ReadFileHeader: Invalid MIDI chunk header! "
                        "chunk_size=%i\n",
                        (int) __builtin_bswap32(file->header.chunk_header.chunk_size));
        return false;
    }

    format_type = __builtin_bswap16(file->header.format_type);
    file->num_tracks = __builtin_bswap16(file->header.num_tracks);
...
midi_file_t *MIDI_LoadFile(char *filename)
{
    ...
    if (fseek(disk, 0, SEEK_END) < 0 || (length = ftell(disk)) <= 0
     || fseek(disk, 0, SEEK_SET) < 0)
    {
        fclose(disk);
        return NULL;
    }
    data = malloc(length);
    if (data == NULL || fread(data, 1, length, disk) != (size_t) length)
    {
        free(data);
        fclose(disk);
        return NULL;
    ...
    fclose(disk);
    result = MIDI_LoadFileMem(data, length);
    free(data);
    return result;
}

midi_file_t *MIDI_LoadFileMem(const void *data, unsigned int len)
{
    midi_file_t *file;
    midi_stream_t stream;
    ...
    if (!ReadFileHeader(file, &stream) || !ReadAllTracks(file, &stream))
    {
        MIDI_FreeFile(file);
        return NULL;
    }
...
{
    short result = __builtin_bswap16(file->header.time_division);
