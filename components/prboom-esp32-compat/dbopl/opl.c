...
// Largest chunk ever requested of FillBuffer() in one call. Bounded by the
// caller's mixer buffer size (see MIX_FRAMES in i_sound.c); asserted below.
#define OPL_MAX_RENDER_SAMPLES 512

static int init_stage_reg_writes = 1;

unsigned int opl_sample_rate = 22050;
// dbopl's raw synthesis output peaks far below full scale (typically a few
// percent of INT16_MAX even for a busy chord), so on its own it is
// inaudible next to the PCM sound effects, which are mixed near full
// volume. Boost it here; 50 would be unity gain.
int mus_opl_gain = 300;
...
// OPL software emulator structure. DOSBox's dbopl is a table/block-based
// OPL2/OPL3 emulator: unlike the cycle-accurate Nuked core, it synthesizes
// directly at the requested output rate (no internal oversampling), which
// is what keeps it cheap enough to run in real time on this MCU. Doom's
// GENMIDI driver only ever talks to the OPL2 register set (channels 0-8),
// so we never switch the chip into OPL3 mode and can render mono.
static Chip opl_chip;
static bool opl_tables_ready;
...
static void WriteRegister(unsigned int reg_num, unsigned int value)
{
    switch (reg_num)
    {
        ...
        default:
            Chip__WriteReg(&opl_chip, reg_num, (Bit8u) value);
            break;
    }
}
...
{
    static int32_t mono[OPL_MAX_RENDER_SAMPLES];
    unsigned int i;
    int32_t sampval;

    assert(nsamples <= OPL_MAX_RENDER_SAMPLES);
    Chip__GenerateBlock2(&opl_chip, nsamples, mono);

    // GENMIDI/AdLib music is inherently mono (real OPL2 has one FM output);
    // duplicate it across both channels the same way the hardware's mono
    // line feeds a stereo amp.
    for (i=0; i<nsamples; ++i)
    {
        sampval = mono[i] * mus_opl_gain / 50;
        if (sampval > 32767)
            sampval = 32767;
