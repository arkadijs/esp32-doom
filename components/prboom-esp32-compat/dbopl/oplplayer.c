...
static void MetaEvent(opl_track_data_t *track, midi_event_t *event)
{
    switch (event->data.meta.type)
    {
        ...
        case MIDI_META_SET_TEMPO:
            ...
            {
                unsigned int usec = (event->data.meta.data[0] << 16)
                                  | (event->data.meta.data[1] << 8)
                                  | event->data.meta.data[2];
                track->ms_per_beat = usec / 1000;
                if (track->ms_per_beat == 0) track->ms_per_beat = 1;
            }
            break;
...
static const void *I_OPL_RegisterSong(const void *data, unsigned len)
{
    ...
    if (len < 100)
    {
        lprintf (LO_WARN, "I_OPL_RegisterSong: Very short MIDI (%u bytes)\n", len);
        return NULL;
    }
